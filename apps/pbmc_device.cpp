...
int main(int argc, const char** argv) {
    try {
        const std::string        original_command_line = uepm::PBMC::command_line_from_arguments(argc, argv);
        ...
        if (!arg_write_basic_config.getValue().empty()) {
            ...
            uepm::PBMC::write_basic_device_pbmc_config(output_file);
            fmt::print("Wrote complete PBMC configuration to {}\n", output_file.string());
            return 0;
        }
        ...
        auto run_config         = uepm::PBMC::load_device_pbmc_config(arg_config.getValue(), arg_overrides.getValue());
