/**
 * @file admc_device.cpp
 * @brief Self-consistent ADMC device simulation executable.
 */
...
int main(int argc, const char** argv) {
    try {
        const std::string        original_command_line = uepm::ADMC::admc_command_line_from_arguments(argc, argv);
        ...
        TCLAP::CmdLine               cmd("Self-consistent ADMC device simulation.", ' ', "1.0");
        ...
        if (!arg_write_basic_config.getValue().empty()) {
            ...
            uepm::ADMC::write_basic_device_admc_config(output_file);
            fmt::print("Wrote complete ADMC configuration to {}\n", output_file.string());
            return 0;
        }
        ...
        auto run_config         = uepm::ADMC::load_device_admc_config(arg_config.getValue(), arg_overrides.getValue());
        run_config.command_line = original_command_line;
        uepm::ADMC::run_self_consistent_device_admc_simulation(run_config);
        return 0;
