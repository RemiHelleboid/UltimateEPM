...
int main(int argc, const char** argv) {
    try {
        ...
        TCLAP::ValueArg<double> arg_temperature("T", "temperature", "Lattice temperature in K.", false, 300.0, "double");
        TCLAP::ValueArg<double> arg_doping(
            "", "doping", "Absolute doping concentration in cm^-3.", false, 1.0e16, "double");
        ...
        config.number_electrons = carrier_type == uepm::ADMC::carrier_type::electron
                                      ? static_cast<std::size_t>(arg_particles.getValue())
                                      : 0;
        config.number_holes = carrier_type == uepm::ADMC::carrier_type::hole
                                  ? static_cast<std::size_t>(arg_particles.getValue())
                                  : 0;
