...
void merge_config(YAML::Node target, const YAML::Node& source, const std::string& path = "") {
    if (!source.IsMap()) {
        throw std::invalid_argument(fmt::format("Configuration section '{}' must be a mapping.",
...
void apply_override(YAML::Node config, const std::string& override_text) {
    ...
    if (equals == std::string::npos || equals == 0 || equals + 1 >= override_text.size()) {
        throw std::invalid_argument(
            fmt::format("Invalid override '{}'. Expected path.to.value=value.", override_text));
    }
...
T value_at(const YAML::Node& config, std::string_view section, std::string_view key) {
    try {
        ...
        throw std::invalid_argument(
            fmt::format("Invalid value for '{}.{}': {}", section, key, error.what()));
    }
}
...
self_consistent_device_amc_run_config load_device_amc_config(
    ...
    result.mesh_file =
        resolve_input_path(config_file, value_at<std::string>(config, "input", "device_mesh"));
    result.material_file =
        resolve_input_path(config_file, value_at<std::string>(config, "input", "material_file"));
    ...
    device.m_stop_simu_when_no_electron_remaining =
        value_at<bool>(config, "simulation", "stop_when_no_electrons");
    ...
    device.m_enable_scheduled_particle_injection =
        value_at<bool>(config, "scheduled_injection", "enabled");
    ...
    injection.m_particle_type =
        parse_particle_type(value_at<std::string>(config, "scheduled_injection", "type"));
    ...
    common.m_initialize_particles_from_doping =
        value_at<bool>(config, "particles", "initialize_from_doping");
    common.m_initial_particle_weight = value_at<double>(config, "particles", "initial_weight");
    common.m_contact_injection_particle_weight =
        value_at<double>(config, "particles", "contact_injection_weight");
    common.m_passive_quench_circuit.m_enabled =
        value_at<bool>(config, "quench_circuit", "enabled");
    common.m_passive_quench_circuit.m_resistance_ohm =
        value_at<double>(config, "quench_circuit", "resistance_ohm");
    common.m_passive_quench_circuit.m_capacitance_F =
        value_at<double>(config, "quench_circuit", "capacitance_F");
    common.m_quench_biased_contact =
        parse_biased_contact(value_at<std::string>(config, "quench_circuit", "biased_contact"));
    common.m_ramo_current_to_quench_current_sign =
        value_at<double>(config, "quench_circuit", "ramo_current_sign");
    common.m_background_ramo_current_A =
        value_at<double>(config, "quench_circuit", "background_ramo_current_A");
    common.m_avalanche_voltage_drop_threshold_V =
        value_at<double>(config, "avalanche_detection", "voltage_drop_V");
    common.m_quench_high_field_threshold_V_per_cm =
        value_at<double>(config, "quench_detection", "high_field_V_per_cm");
    common.m_quench_quiet_time_s = value_at<double>(config, "quench_detection", "quiet_time_s");

    const auto biased_voltage =
        common.m_quench_biased_contact == quench_biased_contact::anode ? common.m_anode_voltage
