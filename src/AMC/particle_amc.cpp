...
void particle_amc::print_info() const {
    ...
    std::cout << "  Position: (" << m_state.position.x() << ", " << m_state.position.y() << ", " << m_state.position.z() << ") m\n";
    std::cout << "  Local k: (" << m_state.local_k.x() << ", " << m_state.local_k.y() << ", " << m_state.local_k.z() << ") 1/m\n";
    std::cout << "  Velocity: (" << m_state.velocity.x() << ", " << m_state.velocity.y() << ", " << m_state.velocity.z() << ") m/s\n";
...
void particle_amc::set_data_from_device(int m_dimension) {
    ...
    if (m_state.m_containing_element != nullptr) {
        ...
        m_state.doping_concentration = m_state.m_containing_element->interpolate_doping_at_location(interp_position);
    } else {
...
void particle_history::export_trajectory_as_csv(const std::string& filename) const {
    ...
    for (const auto& snapshot : m_snapshots) {
        file << snapshot.time << "," << snapshot.position.x() << "," << snapshot.position.y() << "," << snapshot.position.z() << ","
             << snapshot.electric_field_norm << "," << snapshot.kinetic_energy << "," << snapshot.gamma << "," << snapshot.valley_index
             << "\n";
    }
