...
std::size_t particle_scattering_event_count(const pbmc_particle& particle, scattering_event event) {
    const std::size_t event_index = static_cast<std::size_t>(event);
    return particle.history().scattering_events()[event_index];
}

double particle_drift_velocity_along_field_m_per_s(const pbmc_particle& particle, const mesh::vector3& electric_field) {
...
pbmc_transport_config bulk_pbmc_simulation::make_transport_config(const bulk_pbmc_simulation_config& cfg) {
    pbmc_transport_config transport_cfg;
...
std::size_t bulk_pbmc_simulation::count_scattering_events(scattering_event event) const {
    std::size_t       total       = 0;
    const std::size_t event_index = static_cast<std::size_t>(event);
...
double bulk_pbmc_simulation::average_drift_velocity_along_field_m_per_s() const {
...
void bulk_pbmc_simulation::initialize() {
    ...
    for (std::size_t i = 0; i < m_cfg.m_number_of_particles; ++i) {
        pbmc_particle p{i, m_cfg.m_carrier_type, 1.0};
...
void bulk_pbmc_simulation::accumulate_observables(double dt) {
    if (dt <= 0.0) {
        return;
    }
...
void bulk_pbmc_simulation::accumulate_particle_observables(const pbmc_particle& p, double dt) {
    if (dt <= 0.0) {
        return;
    }
    m_observables.weighted_velocity_x_m2_per_s2 += p.state().velocity.x() * dt;
...
void bulk_pbmc_simulation::run_self_scattering_emc() {
    ...
    std::vector<pbmc_transport_kernel> thread_transports;
...
void bulk_pbmc_simulation::export_particles_history_to_csv(const std::string& prefix_name) const {
    for (const auto& p : m_particles) {
