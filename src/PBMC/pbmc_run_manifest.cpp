/**
 * @file pbmc_run_manifest.cpp
 * @brief Utilities for exporting reproducible PBMC run metadata.
 */
...
void simulation_manifest::write(const std::filesystem::path& filename) const {
    ...
    stream << "# UltimateEPM PBMC simulation manifest\n";
...
std::string pbmc_project_version() { return UEPM_PROJECT_VERSION; }

std::string pbmc_build_type() { return UEPM_BUILD_TYPE; }

std::string pbmc_compiler() { return UEPM_COMPILER; }
