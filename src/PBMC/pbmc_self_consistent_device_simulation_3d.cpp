/**
 * @file pbmc_self_consistent_device_simulation_3d.cpp
...
void self_consistent_device_pbmc_simulation_3d::validate_self_consistent_options() const {
    if (m_dimension != 3) {
        throw std::invalid_argument("self_consistent_device_pbmc_simulation_3d requires a 3D device.");
    }
 ...
 *
 */
void self_consistent_device_pbmc_simulation_3d::initialize_contact_elements() {
...
void self_consistent_device_pbmc_simulation_3d::place_initial_charges_according_to_doping(double particle_weight) {
    ...
    constexpr double min_probability = 25e-2;
...
void self_consistent_device_pbmc_simulation_3d::add_charges_at_contacts(std::size_t poisson_frequency) {
    if (poisson_frequency == 0) {
        throw std::invalid_argument("Poisson frequency must be positive.");
    }
...
void self_consistent_device_pbmc_simulation_3d::compute_unitary_potential() {
    ...
    if (std::abs(unitary_potential_max - unitary_potential_min) < 1e-6) {
        ...
        m_state.m_RamoUnitaryElectricField_Vm_per_cm =
            bulk_elements.front()->interpolate_vector_at_location("RamoUnitaryPotential_gradient",
                                                                  bulk_elements.front()->get_barycenter());
    } else {
...
void self_consistent_device_pbmc_simulation_3d::initialize_poisson_solver() {
...
void self_consistent_device_pbmc_simulation_3d::initialize_particles_for_self_consistent_run() {
...
void self_consistent_device_pbmc_simulation_3d::add_particle_charges_to_elements() {
...
void self_consistent_device_pbmc_simulation_3d::update_self_consistent_potential() {
...
void self_consistent_device_pbmc_simulation_3d::run_self_consistent_transport_simulation() {
    ...
    fmt::print("START 3D SELF-CONSISTENT PBMC SIMULATION\n");
    ...
    while (m_state.m_time_s <= m_simulation_options.m_t_max && !m_list_particles.empty()) {
        ...
        if (should_update_poisson) {
            ...
            if (m_common_options.m_auto_background_ramo_current && m_state.m_scheduled_particle_injection_done &&
                ...
                const double time_window = 1e-12;
                const double bg_current  = m_simulation_history.extract_final_current(time_window);
    ...
    stream.close();
    export_current_state();
    fmt::print("END 3D SELF-CONSISTENT PBMC SIMULATION\n");
}

void self_consistent_device_pbmc_simulation_3d::export_current_state() { export_current_snapshot(); }
