...
Single_particle_simulation::Single_particle_simulation(uepm::mesh_bz::ElectronPhonon* ptr_mesh_bz,
    ...
    for (std::size_t i = 0; i < m_nb_particles; ++i) {
        ...
        particle.state().m_position = {0.0, 0.0, 0.0};
    ...
    for (auto& particle : m_list_particle) {
        particle.state().m_energy   = thermal_energy;
        particle.state().m_velocity = {0.0, 0.0, 0.0};
    }
...
#pragma omp parallel for
    for (auto& particle : m_list_particle) {
        vector3 initial_k =
            m_ptr_mesh_bz->draw_random_k_point_at_energy(thermal_energy, 0, particle.get_random_generator());
        particle.state().m_k_vector = initial_k;
        std::cout << "Initial k-vector (drawn at thermal energy): " << initial_k << "\n";
    }

    // Attach containing tetra
    for (auto& particle : m_list_particle) {
        uepm::mesh_bz::Tetra* containing_tetra = m_ptr_mesh_bz->find_tetra_at_location(particle.state().m_k_vector);
        if (containing_tetra == nullptr) {
            throw std::runtime_error("Initial k-point is out of the Brillouin zone mesh.");
        }
        // particle.state().m_containing_bz_mesh_tetra = containing_tetra;
        particle.set_containing_bz_mesh_tetra(containing_tetra);
        // Physical total rate at current k
        std::array<double, 8> rates =
            particle.interpolate_phonon_scattering_rate_at_location(particle.state().m_k_vector);
        const double Gamma       = std::accumulate(rates.begin(), rates.end(), 0.0);
        particle.state().m_gamma = Gamma;

        // Sync velocity & energy
        particle.update_group_velocity();
        particle.update_energy();

        const double init_energy_true = containing_tetra->interpolate_energy_at_band(particle.state().m_k_vector, 0);

        fmt::print("Initial energy (set):  {:.10f} eV\n", thermal_energy);
        fmt::print("Initial energy (true): {:.10f} eV\n", init_energy_true);

        particle.state().m_energy = init_energy_true;
...
#pragma omp critical
                fmt::print("Particle {} at time {:.3e} / {:.3e} s, iteration {}, energy {:.4f} eV\n",
                           particle.get_index(),
                           particle.state().m_time,
                           T_end,
                           particle.state().m_iter,
                           particle.state().m_energy);
            }
            ...
            uepm::mesh_bz::Tetra* containing_tetra = m_ptr_mesh_bz->find_tetra_at_location(particle.state().m_k_vector);
            if (containing_tetra == nullptr) {
                const vector3 folded_k = m_ptr_mesh_bz->retrieve_k_inside_mesh_geometry(particle.state().m_k_vector);
                particle.state().m_k_vector = folded_k;
                containing_tetra            = m_ptr_mesh_bz->find_tetra_at_location(particle.state().m_k_vector);
                if (containing_tetra == nullptr) {
            ...
            const double energy_particle = particle.state().m_energy;
            ...
            const double energy_tetra = containing_tetra->interpolate_energy_at_band(particle.state().m_k_vector,
                                                                                     particle.state().m_band_index);
