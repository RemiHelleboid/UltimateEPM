...
TEST_CASE("screened two-body matrix element enforces momentum conservation") {
    ...
    const auto matrix_element = uepm::mesh_bz::compute_screened_two_body_matrix_element(
        initial_a, initial_b, final_a, final_b, basis, interaction, config);
    CHECK(matrix_element.real() == doctest::Approx(2.0));
    CHECK(matrix_element.imag() == doctest::Approx(1.0));

    const auto momentum_forbidden = uepm::mesh_bz::compute_screened_two_body_matrix_element(
        initial_a, initial_b, final_a, make_state(0.5, 1.0, 0), basis, interaction, config);
...
TEST_CASE("direct exchange matrix element swaps final electrons") {
    ...
    const auto [direct, exchange] = uepm::mesh_bz::compute_direct_exchange_impact_ionization_matrix_element(
        initial_hot, initial_valence, final_1, final_2, basis, interaction, config);
...
TEST_CASE("screened two-body matrix element validates inputs") {
    ...
    const auto interaction = [](const uepm::mesh_bz::vector3&, double) {
        return uepm::mesh_bz::complex_d{1.0, 0.0};
    };
    auto state = make_state(0.0, 0.0, 0);
    state.coefficients = Eigen::VectorXcd::Ones(2);

    CHECK_THROWS_AS(uepm::mesh_bz::compute_screened_two_body_matrix_element(
                        state, state, state, state, basis, interaction),
    ...
    CHECK_THROWS_AS(uepm::mesh_bz::compute_screened_two_body_matrix_element(
                        valid_state, valid_state, valid_state, valid_state, basis, interaction, bad_config),
                    std::invalid_argument);
}
