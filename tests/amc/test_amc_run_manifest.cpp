...
TEST_CASE("simulation manifest groups sections and preserves precise values") {
    ...
    const auto filename =
        std::filesystem::temp_directory_path() / "ultimate_epm_amc_manifest_test.txt";
...
TEST_CASE("command line reconstruction quotes arguments containing spaces") {
    ...
    CHECK(uepm::amc::command_line_from_arguments(3, arguments) ==
          "device_amc.epm --outdir \"result directory\"");
}
