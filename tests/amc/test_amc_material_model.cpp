...
TEST_CASE("silicon material model preserves scattering parameters") {
    ...
    const auto material                          = uepm::amc::make_silicon_amc_material_model(common_material);
...
TEST_CASE("parameterized rates reproduce silicon reference values") {
    ...
    const double electron_acoustic = uepm::amc::acoustic_scattering_rate(material.m_electron_valleys.front(),
    ...
    const double hole_acoustic =
        uepm::amc::acoustic_scattering_rate(material.m_hole_bands.front(), material.m_hole_acoustic, 0.1, 300.0);
    ...
    const double g3_absorption =
        uepm::amc::intervalley_scattering_rate(material.m_electron_valleys.front(),
                                               g3_transition,
    ...
    const double screened_impurity =
        uepm::amc::screened_coulomb_impurity_momentum_relaxation_rate(material.m_electron_valleys.front(),
...
TEST_CASE("finite-temperature impurity screening matches Dawson references") {
    ...
    CHECK(uepm::amc::impurity_screening_function(0.1) == doctest::Approx(0.9933599239785286).epsilon(2.0e-7));
    CHECK(uepm::amc::impurity_screening_function(0.5) == doctest::Approx(0.8488727670040446).epsilon(2.0e-7));
    CHECK(uepm::amc::impurity_screening_function(1.0) == doctest::Approx(0.5380795069127684).epsilon(2.0e-7));
    CHECK(uepm::amc::impurity_screening_function(4.0) == doctest::Approx(0.03233700030900128).epsilon(2.0e-7));
    CHECK(uepm::amc::impurity_screening_function(10.0) == doctest::Approx(0.005025384718759853).epsilon(2.0e-6));
}
...
TEST_CASE("full impurity screening is finite across the validation grid") {
    ...
    for (double energy_eV : energies_eV) {
        for (double density_cm_3 : densities_cm_3) {
            const double analytic_rate = uepm::amc::screened_coulomb_impurity_momentum_relaxation_rate(
                valley,
                material.m_dielectric.epsilon_r,
                energy_eV,
                density_cm_3,
            ...
            const double full_rate = uepm::amc::screened_coulomb_impurity_momentum_relaxation_rate(
                valley,
                material.m_dielectric.epsilon_r,
                energy_eV,
                density_cm_3,
...
TEST_CASE("transition names travel with channels and are recorded") {
    ...
    uepm::amc::amc_transport_kernel hole_transport(config, uepm::amc::make_silicon_amc_material_model(), 5678);
