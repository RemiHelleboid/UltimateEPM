        ...
        for (int ix = -1; ix <= 1; ++ix) {
            for (int iy = -1; iy <= 1; ++iy) {
                for (int iz = -1; iz <= 1; ++iz) {
                    ...
                    const double           energy =
                        alpha_eV_inv > 0.0 ? (std::sqrt(1.0 + 4.0 * alpha_eV_inv * gamma) - 1.0) / (2.0 * alpha_eV_inv)
                                                     : gamma;
...
std::vector<uepm::pseudopotential::valley_fit_sample> make_synthetic_kane_samples(
    ...
    double                                alpha_eV_inv,
    uepm::pseudopotential::band_edge_kind edge_kind) {
    return make_synthetic_kane_samples(k0,
                                       lattice_constant_m,
                                       masses_m0,
                                       alpha_eV_inv,
                                       {0.006, 0.012, 0.018},
...
TEST_CASE("effective-mass fit recovers anisotropic Kane parameters around a minimum") {
    ...
    const auto                  samples = make_synthetic_kane_samples(k0,
                                                     lattice_constant_m,
                                                     masses,
                                                     alpha,
    ...
    const auto result =
        uepm::pseudopotential::fit_effective_mass_and_nonparabolicity(samples,
                                                                      k0,
                                                                      1.25,
                                                                      lattice_constant_m,
                                                                      uepm::pseudopotential::band_edge_kind::minimum);
...
TEST_CASE("effective-mass fit supports valence-like maxima") {
    ...
    const auto                  samples = make_synthetic_kane_samples(k0,
                                                     ...
                                                     masses,
                                                     alpha,
                                                     uepm::pseudopotential::band_edge_kind::maximum);
...
TEST_CASE("split fit keeps near-edge masses while fitting alpha over a wider energy window") {
    ...
    const auto                  mass_samples  = make_synthetic_kane_samples(k0,
                                                          lattice_constant_m,
                                                          masses,
                                                          alpha,
                                                                            {0.001, 0.002, 0.003},
                                                          uepm::pseudopotential::band_edge_kind::minimum);
    const auto                  alpha_samples = make_synthetic_kane_samples(k0,
                                                           lattice_constant_m,
                                                           masses,
                                                           alpha,
                                                                            {0.006, 0.012, 0.018},
...
TEST_CASE("fixed-mass alpha fit can clamp unphysical negative alpha") {
    ...
    auto mass_fit = uepm::pseudopotential::fit_effective_mass_tensor(samples,
                                                                     k0,
                                                                     1.25,
                                                                     lattice_constant_m,
                                                                     uepm::pseudopotential::band_edge_kind::minimum);
