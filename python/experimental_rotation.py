...
def plot_iso_surface(filename_vtu, band_index, min_energy, max_energy, number_iso_values, out_file, nb_frames):
    ...
    # set scalar coloring
    ColorBy(medium_1_bz_meshvtuDisplay, ('POINTS', band_color))
    ...
    band_0LUT = GetColorTransferFunction(band_color)

    # get opacity transfer function/opacity map for band_str
    band_0PWF = GetOpacityTransferFunction(band_color)
    ...
    # set scalar coloring
    ColorBy(contour1Display, ('POINTS', band_color))
    
        # get color transfer function/color map for 'band_0'
    band_0LUT = GetColorTransferFunction(band_color)

    # Apply a preset using its name. Note this may not work as expected when presets have duplicate names.
    band_0LUT.ApplyPreset('Plasma (matplotlib)', True)
